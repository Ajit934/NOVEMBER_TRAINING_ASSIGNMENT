...
public class Q_2_Employee {
	public static void main(String[] args) throws Q_2_LeaveExceedLimitException, Q_2_EmployeeAbscondingException {
		double salary = 25000;
		...
		if (absent < 1) {
			if (noInfo >= 4) {
				if (isSequence(noinfoArr)) {
					...
					throw new Q_2_EmployeeAbscondingException("Your salary will not be calculated");
				}
			} else if (leave > 2) {
				System.out.println("Total salary:" + salaryCal(salary, present - (leave - 2)));

				throw new Q_2_LeaveExceedLimitException("Leave balance exceeded");
